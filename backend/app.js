...
mongoose.connect('mongodb://127.0.0.1:27017/ecomerceKle')
.then(()=>{
...
// for form method we uae middleware
...
app.post('/register',async(req,res)=>{
    ...
    //to check use  is register or not
    const isUserAlreadyExsit = await User.findOne({email});

    if(isUserAlreadyExsit){
        res.status(400).json({message:"User Already hava a Account "})
        return;
    }else{
        //hashing password
